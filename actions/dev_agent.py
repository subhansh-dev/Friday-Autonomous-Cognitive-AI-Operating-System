...
# [FIX-10] Known module → pip package aliases
_PACKAGE_ALIASES = {
    "pil": "Pillow",
    "cv2": "opencv-python",
...
# [FIX-1] Normalize paths before comparing
def _parse_traceback(output: str, project_files: list[str]) -> tuple[str | None, int | None]:
    ...
    for raw_path, line_str in reversed(matches):
        ...
        for pf in project_files:
            ...
            pf_norm = pf.replace("\\", "/")
            if pf_name == raw_name or pf == raw_path or \
               raw_norm.endswith(pf_norm) or pf_norm == raw_norm:
                return pf, int(line_str)

    return None, None


# [FIX-2] Remove duplicate importerror check, reorder
...
# [FIX-6] Removed unused run_command parameter
...
JSON:"""
    ...
    try:
        raw = _generate(MODEL_PLANNER, prompt)
...
Code for {file_path}:"""
    ...
    try:
        code = _generate(MODEL_WRITER, prompt)
...
# [FIX-9] Use string with shell=True, not list
...
# [FIX-8] Handle python3, add shell flag for Windows
...
# [FIX-10] Alias map for common module → pip package mismatches
...
# [FIX-4] Use MODEL_WRITER for code fixing
# [FIX-5] Fix both the broken file AND its dependencies/consumers
def _fix_files(
    error_output: str,
    project_description: str,
    all_files: list[dict],
    ...
    if error_file:
        ...
        if error_type == "import_error":
            # Fix the source module that the broken file tries to import from
            error_file_info = next(
                (f for f in all_files if f["path"] == error_file), None
            )
            if error_file_info:
                for imp in error_file_info.get("imports", []):
                    imp_path = imp.replace(".", "/") + ".py"
                    if imp_path not in files_to_fix and imp_path in file_codes:
                        files_to_fix.append(imp_path)

            # Also fix files that import the broken file (cascading)
...
Fixed code for {fix_path}:"""
        ...
        try:
            # [FIX-4] Use MODEL_WRITER for code generation/fixing
            fixed = _generate(MODEL_WRITER, prompt)
...
# [FIX-7] Topological sort for correct build order
...
def _build_project(
    ...
    # [FIX-7] Topological sort instead of naive import-count sort
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        # [FIX-6] Removed unused run_command arg
