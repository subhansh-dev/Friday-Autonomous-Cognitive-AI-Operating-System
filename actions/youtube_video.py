...
# [FIX-10] Lazy imports — don't fail at module load
_pyautogui = None
_np = None
...
# [FIX-1] Use platform module directly instead of broken config import
...
def _scrape_first_video_url(query: str) -> str | None:
    ...
    try:
        ...
        for vid in video_ids:
            ...
            # [FIX-7] Better Shorts detection
            if f'/shorts/{vid}' in html or f'"reelWatchEndpoint"' in html:
                continue
...
# [FIX-2] + [FIX-3] Safe URL input — no Tkinter on worker threads
def _ask_for_url(prompt_text: str = "YouTube video URL:") -> str | None:
    """Prompt user for a URL. Uses stdin if on a worker thread."""
    import threading
    if threading.current_thread() is not threading.main_thread():
        # Can't use Tkinter from a worker thread
        print(f"[YouTube] {prompt_text}")
        try:
            url = input("URL> ").strip()
    ...
    try:
        ...
        url = simpledialog.askstring("FRIDAY", prompt_text, parent=root)
...
def _get_transcript(video_id: str) -> str | None:
    ...
    try:
        ...
        lang_priority = ["en", "tr", "de", "fr", "es", "it",
                         "pt", "ru", "ja", "ko", "ar", "zh"]
...
def _summarize_with_gemini(transcript: str, video_url: str) -> str:
    ...
    sys_instr = (
        ...
        "Summarize YouTube transcripts with clarity and wit. "
        "Structure: 1-sentence overview, then 3-5 key points. "
        "Be direct. Call the user 'Sir'."
    )
    ...
    # [FIX-9] Timeout via request options
...
# [FIX-4] More resilient video info scraping
def _scrape_video_info(video_id: str) -> dict:
    ...
    try:
        ...
        # [FIX-4] Multiple pattern variants for each field
        patterns = {
            "title": [
                r'"title":\{"runs":$$\{"text":"([^"]+)"',
                r'"title":"([^"]+)"',
...
# [FIX-5] Configurable region default
...
def _handle_trending(parameters: dict, player, speak) -> str:
    # [FIX-5] Default to US instead of TR
...
def youtube_video(
    ...
    try:
        ...
        # [FIX-8] Always pass speak, even if None
