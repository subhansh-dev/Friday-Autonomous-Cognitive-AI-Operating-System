"""
browser_control.py — FRIDAY Browser Action Module
====================================================
Supported browsers : chrome, edge, firefox, opera, operagx, brave, vivaldi, safari (macOS)
Supported platforms : Windows · macOS · Linux

Each browser gets an independent _BrowserSession instance.
Use "switch" action to change the active browser.

NOTE: All browsers use the user's real profile (accounts, cookies preserved).
      Incognito/private mode is NOT used by default.
"""
...
# ── Firefox real profile directory ────────────────────────────────────────────
def _firefox_profile_dir() -> Optional[str]:
    ...
    # Check the last section after loop ends
    p = current.get("Path", "")
    if p and current.get("Default") == "1":
        is_rel = current.get("IsRelative", "1") == "1"
        default_path = str(base / p) if is_rel else p
...
# ── General exe search (Windows registry) ────────────────────────────────────
...
# [FIX-8] Safari as macOS fallback
    ...
    try:
        if _OS == "Windows":
            ...
            for kw in ("firefox", "opera", "brave", "vivaldi", "safari", "chrome", "edge"):
                if kw in out:
                    return kw
            return "safari"  # [FIX-8] macOS default
        elif _OS == "Linux":
            out = subprocess.run(
...
class _BrowserSession:
    ...
    def _run_loop(self):
        ...
        # [FIX-1] Clean up loop after run_forever returns
        try:
            self._loop.close()
        except Exception:
            pass
    ...
    # [FIX-3] Cancel coroutine on timeout
    def run(self, coro, timeout: int = 60) -> str:
        if not self._loop or not self._loop.is_running():
            raise RuntimeError(f"Session for '{self.browser_name}' not started or already closed.")
    ...
    # [FIX-5] Handle already-stopped loop
    def close(self):
        if self._loop and self._loop.is_running():
            try:
                asyncio.run_coroutine_threadsafe(self._async_close(), self._loop).result(10)
            except Exception as e:
                print(f"[Browser] Close error (non-fatal): {e}")
                # Force stop the loop if _async_close failed
                try:
                    self._loop.call_soon_threadsafe(self._loop.stop)
                except Exception:
                    pass

    # [FIX-1] Stop the event loop so the thread can exit
    async def _async_close(self):
        try:
            ...
            # Schedule loop stop — this causes run_forever() to return
            if self._loop and self._loop.is_running():
                self._loop.call_soon_threadsafe(self._loop.stop)
    ...
    async def _launch(self):
        if self._context is not None:
            ...
            try:
                ...
                print(f"[Browser] Context dead — relaunching {self.browser_name}")
                # [FIX-11] Attempt cleanup of old resources
                try:
                    await self._context.close()
                except Exception:
                    pass
        ...
        if engine_name == "firefox":
            ...
            try:
                ...
                friday = str(Path.home() / ".friday_profiles" / "firefox_friday")
                Path(friday).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(friday, **kwargs)
        ...
        # [FIX-9] Added --disable-extensions for stability
        chromium_args = [
            "--start-maximized",
        ...
        friday_profile = str(Path.home() / ".friday_profiles" / self.browser_name)
        Path(friday_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with FRIDAY profile: {friday_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(friday_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] [OK] Launched [{label}] with FRIDAY profile")
    ...
    async def _get_page(self) -> Page:
        ...
        if self._page is None or self._page.is_closed():
            try:
                # [FIX-6] Verify context is alive before creating new page
                ...
                # context is fully dead — relaunch everything
                self._context = None
                self._page    = None
                await self._launch()
        return self._page

    # [FIX-4] Simplified retry condition
    ...
    # [FIX-10] Better error when no element is focused
        ...
        try:
            if selector:
                ...
                if await focused.count() == 0:
                    return "No element focused and no selector provided. Click an input first."
                el = focused
            if clear_first:
                await el.clear()
    ...
    # [FIX-6] Better cleanup when last tab is closed
    async def close_tab(self) -> str:
        ...
        if page and not page.is_closed():
            ...
            if pages:
                ...
                # Last tab closed — context is empty, clear page reference
                self._page = None
                print(f"[Browser] Last tab closed for {self.browser_name}")
            return "Tab closed."
    ...
    async def reload(self) -> str:
        try:
            ...
            # context died — relaunch and try once more
            print(f"[Browser] Reload failed ({e}) — relaunching context")
            try:
                self._context = None
                self._page    = None
                page = await self._get_page()
...
class _SessionRegistry:
    ...
    # [FIX-2] Don't hold the lock during blocking start()
    def _get_or_create(self, browser_name: str, incognito: bool = False) -> _BrowserSession:
        with self._lock:
            ...
            # Create session object and register it immediately
            # so other threads see it and don't duplicate
            sess = _BrowserSession(browser_name, incognito=incognito)
            self._sessions[browser_name] = sess

        # Start outside the lock — this can block for up to 30s
        sess.start()
    ...
    def get(self, browser_name: str | None = None, incognito: bool = False) -> _BrowserSession:
        ...
        sess = self._get_or_create(browser_name, incognito=incognito)
    ...
    # [FIX-12] Handle individual close failures without blocking others
    def close_all(self) -> str:
        ...
        for s in sessions:
            try:
                s.close()
                closed.append(s.browser_name)
            except Exception as e:
                print(f"[Registry] Error closing {s.browser_name}: {e}")
        return "All browsers closed: " + (", ".join(closed) if closed else "none")
...
def browser_control(
    ...
    try:
        # [FIX-7] Pass incognito flag from parameters
        incognito = bool(params.get("incognito", False))
        sess = _registry.get(browser, incognito=incognito)
    except Exception as e:
